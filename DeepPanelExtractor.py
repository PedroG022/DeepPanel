from ConnectedComponents import ConnectedComponentResult, DeepPanelResult, Panel, find_components 
...
def extract_panels_data(connected_components_result: ConnectedComponentResult, width: int, height: int, scale: float, original_image_width: int, original_image_height: int) -> DeepPanelResult:
    ...
    for i in range(number_of_panels):
        i += 1
        proposed_top = apply_scale_and_add_border(min_x_values[i], scale, -border) - vertical_correction
        proposed_left = apply_scale_and_add_border(min_y_values[i], scale, -border) - horizontal_correction
        proposed_height = apply_scale_and_add_border(max_x_values[i], scale, border) + vertical_correction    
        proposed_width = apply_scale_and_add_border(max_y_values[i], scale, border) - horizontal_correction

        panel = Panel()
        panel.top = clamp(proposed_top, 0, original_image_height)
        panel.left = clamp(proposed_left, 0, original_image_width)
        panel.height = clamp(proposed_height, 0, original_image_height)
        panel.width = clamp(proposed_width, 0, original_image_width)
        panels.append(panel)
...
def extract_panels_info(labeled_matrix, width: int, height: int, scale: float, original_image_width: int, original_image_height: int) -> DeepPanelResult:
    improved_areas_result = find_components(labeled_matrix)
...
def extract_panels(matrix, original_width, original_height):
    ...
    scale = max(original_height, original_width) / float(224)
